...
const SoapNote: React.FC<SoapNoteProps> = ({ isVisible }) => {
  ...
  const handlePdfDownload = () => {
    try {
      const pdf = new jsPDF();
      const splitText = pdf.splitTextToSize(markdown, 180);
  ...
  return (
    <div className={`soap-note-container ${!isVisible ? 'hidden' : ''}`}>
...
function generateSoapNoteMarkdown(data: any): string {
  ...
  return `MEDICAL NOTE
${formatSection('Date', new Date().toLocaleDateString())}
${formatSection('Chief Complaint', data.chiefComplaint)}
${formatSection('History of Present Illness', data.historyOfPresentIllness)}
${formatSection('Past Medical History', data.pastMedicalHistory)}
${formatSection('Medications', data.medications)}
${formatSection('Allergies', data.allergies)}
${formatSection('Review of Systems', data.reviewOfSystems)}
${formatSection('Physical Exam', data.physicalExam)}
${formatSection('Assessment', data.assessment)}
${formatSection('Plan', data.plan)}
${data.icdCodes?.length ? 'ICD-10 Codes:\n' + data.icdCodes.map(
  (code: {code: string, description: string}) => `${code.code} - ${code.description}`
).join('\n') : ''}
